...
//        self.tabBarController?.tabBar.hidden = false
        ...
        let userJSON = defaultToken.objectForKey("userJSONTest")
        if (userJSON == nil) {
            ...
            rest.EnrolledName({(request) in
                defaultToken.setObject(try! request.rawData(), forKey: "userJSONTest")
                self.refreshUI(request)
            })
        }
        else{
            refreshUI(JSON(data: (userJSON as! NSData)))
        }
    }
    
    func refreshUI(request: JSON) {
        ...
        dispatch_async(dispatch_get_main_queue(), {
            ...
            if request != 1 {
                ...
                //TODO: remove below line after completing development
                //self.hasWellness = "False"
